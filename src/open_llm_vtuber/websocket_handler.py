...
from .pet.runtime import PetRuntime
from .pet.state import PetAction
...
class WebSocketHandler:
    ...
    def __init__(
        ...
        pet_runtime: PetRuntime | None = None,
    ):
        ...
        self.pet_runtime = pet_runtime or PetRuntime()
    ...
    def _init_message_handlers(self) -> Dict[str, Callable]:
        ...
        return {
            ...
            "pet-state-request": self._handle_pet_state_request,
            "pet-action": self._handle_pet_action,
            "pet-memory-save": self._handle_pet_memory_save,
            "pet-memory-recall": self._handle_pet_memory_recall,
            "pet-skill-letter": self._handle_pet_skill_letter,
            "pet-skill-drawing": self._handle_pet_skill_drawing,
            "pet-observe": self._handle_pet_observe,
            "pet-persona-switch": self._handle_pet_persona_switch,
        }
    ...
    async def _send_initial_messages(
        ...
        await self._send_pet_state(websocket, "demo")
        await websocket.send_text(
            json.dumps(
                {
                    "type": "persona-switched",
                    "persona": asdict(self.pet_runtime.persona),
                },
                ensure_ascii=False,
            )
        )
    ...
    async def _handle_conversation_trigger(
        ...
        if data.get("type") == "text-input":
            local_response = self.pet_runtime.respond_to_text(
                "demo", str(data.get("text") or "")
            )
            if local_response is not None:
                await websocket.send_text(
                    json.dumps(
                        {
                            "type": "full-text",
                            "text": local_response.message,
                            "source": "pet-domain",
                        },
                        ensure_ascii=False,
                    )
                )
                if local_response.action is not None:
                    await websocket.send_text(
                        json.dumps(
                            {
                                "type": "pet-action-result",
                                "action": local_response.action.action.value,
                                "message": local_response.action.message,
                                "state": asdict(local_response.action.state),
                            },
                            ensure_ascii=False,
                        )
                    )
    ...
    async def _send_pet_state(self, websocket: WebSocket, pet_id: str) -> None:
        await websocket.send_text(
            json.dumps(
                {
                    "type": "pet-state-updated",
                    "state": asdict(self.pet_runtime.world.get_state(pet_id)),
                },
                ensure_ascii=False,
            )
        )

    async def _handle_pet_state_request(
        self, websocket: WebSocket, client_uid: str, data: WSMessage
    ) -> None:
        """Send the latest server-owned pet state to the client."""
        await self._send_pet_state(websocket, str(data.get("pet_id") or "demo"))

    async def _handle_pet_action(
        self, websocket: WebSocket, client_uid: str, data: WSMessage
    ) -> None:
        """Execute an explicit pet action and return its result."""
        action = PetAction(str(data.get("action")))
        result = self.pet_runtime.world.perform_action(
            str(data.get("pet_id") or "demo"), action
        )
        await websocket.send_text(
            json.dumps(
                {
    ...
    async def _handle_pet_memory_save(
        self, websocket: WebSocket, client_uid: str, data: WSMessage
    ) -> None:
        """Save one explicit owner fact."""
        record = self.pet_runtime.remember(
            str(data.get("pet_id") or "demo"),
            str(data.get("key") or ""),
            str(data.get("value") or ""),
        )
        await websocket.send_text(
            json.dumps(
                {"type": "pet-memory-saved", "memory": asdict(record)},
                ensure_ascii=False,
            )
        )

    async def _handle_pet_memory_recall(
        self, websocket: WebSocket, client_uid: str, data: WSMessage
    ) -> None:
        """Recall owner facts for the client or prompt layer."""
        records = self.pet_runtime.memory.recall(
            str(data.get("pet_id") or "demo"), str(data.get("query") or "")
        )
        await websocket.send_text(
            json.dumps(
                {
                    "type": "pet-memory-results",
    ...
    async def _handle_pet_skill_letter(
        self, websocket: WebSocket, client_uid: str, data: WSMessage
    ) -> None:
        """Generate and return a local Markdown letter."""
        artifact = self.pet_runtime.artifacts.write_letter(
            str(data.get("pet_id") or "demo"),
            str(data.get("recipient") or "主人"),
            str(data.get("content") or ""),
            str(data.get("title") or "给主人的信"),
        )
        await self._send_artifact(websocket, artifact)

    async def _handle_pet_skill_drawing(
        self, websocket: WebSocket, client_uid: str, data: WSMessage
    ) -> None:
        """Generate and return an offline SVG drawing."""
        artifact = self.pet_runtime.artifacts.create_drawing(
            str(data.get("pet_id") or "demo"),
            str(data.get("content") or "和主人一起发光"),
            str(data.get("title") or "宠物画作"),
        )
        await self._send_artifact(websocket, artifact)

    async def _send_artifact(self, websocket: WebSocket, artifact) -> None:
        await websocket.send_text(
            json.dumps(
                {
                    "type": "pet-artifact-created",
                    "artifact": {
                        **asdict(artifact),
                        "path": f"/pet-artifacts/{artifact.path.name}",
                    },
                },
                ensure_ascii=False,
            )
    ...
    async def _handle_pet_observe(
        self, websocket: WebSocket, client_uid: str, data: WSMessage
    ) -> None:
        """Evaluate a camera event and trigger a safe proactive message."""
        decision = self.pet_runtime.observe(str(data.get("event_type") or ""))
        await websocket.send_text(
            json.dumps(
                {"type": "pet-proactive-decision", "decision": asdict(decision)},
                ensure_ascii=False,
            )
        )
    ...
    async def _handle_pet_persona_switch(
        self, websocket: WebSocket, client_uid: str, data: WSMessage
    ) -> None:
        """Switch persona and broadcast its media contract."""
        persona = self.pet_runtime.switch_persona(str(data.get("persona_id")))
        context = self.client_contexts.get(client_uid)
        if context is not None:
            await context.init_agent(
                context.character_config.agent_config,
                self.pet_runtime.build_prompt("demo"),
            )
        await websocket.send_text(
            json.dumps(
                {"type": "persona-switched", "persona": asdict(persona)},
                ensure_ascii=False,
            )
        )
